...
class DiceEstimator(Estimator):
    ...
    def get_estimate(self, data: torch.Tensor, output: torch.Tensor) -> torch.Tensor:
        ...
        # Stack counterfactuals into a tensor of shape (batch_size, K, input_dim)
        cfs_tensor = torch.stack(cfs)  # shape: (batch_size, K, input_dim)

        # Expand data to match cfs_tensor shape for broadcasting
        data_expanded = data.unsqueeze(1)  # shape: (batch_size, 1, input_dim)

        # Compute L2 distances between each input and its K counterfactuals
        l2_distances = torch.norm(cfs_tensor - data_expanded, p=2, dim=-1)  # shape: (batch_size, K)

        # Compute mean over K counterfactuals for each input
        mean_l2_per_input = l2_distances.mean(dim=1)  # shape: (batch_size,)
        assert mean_l2_per_input.shape == (data.shape[0],), "Output shape mismatch"
        return mean_l2_per_input